...
export const getShipmentHealthData = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    ...
    // Verify ownership (only the assigned hub can view details)
    if (shipment.hub_id.toString() !== req.user.id) {
      res.status(403).json({ message: 'Not authorized' });
      return;
    }

    // Call Health Microservice
    try {
      const healthRes = await axios.get(`${HEALTH_URL}/api/internal/interventions/${shipment.health_request_id}`, {
        headers: { 'x-api-key': SHARED_SECRET }
      });
      
      res.json(healthRes.data);
    } catch (healthError: any) {
      console.error("Health Service Error:", healthError.message);
      res.status(502).json({ message: 'Failed to fetch data from Health Service' });
    }
